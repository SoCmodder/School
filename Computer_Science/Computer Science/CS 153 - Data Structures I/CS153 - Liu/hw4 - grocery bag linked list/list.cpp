...
void LINKLIST::insertAtHead( const GroceryItem & item1 )
{
  NODE *newNode = new NODE( item1, NULL );
  newNode->setLink( headPtr ); // Links the new NODE to what headPtr WAS pointing at.
  headPtr = newNode;
}
...
void LINKLIST::removeNode( NODE* ptrToPrevNode )
{
  if( ptrToPrevNode != NULL) {
    NODE* temp = ptrToPrevNode->getLink(); // Sets temp = to the node you want to delete.
    ptrToPrevNode->setLink(temp->getLink()); // Sets temp's link to the node after you want to delete.
...
{ 
     ...
     for( temp = headPtr; temp != NULL; temp = temp->getLink() ) 
     { 
          // Converts price and description into one string and adds
          // it to the listbox.
          listBoxControl.AddString( temp->getItem().toString() ); 
     }     
}
