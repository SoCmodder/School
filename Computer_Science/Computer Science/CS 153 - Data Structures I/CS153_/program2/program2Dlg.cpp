...
void CTrashBagDlg::updateTrashBagContentsDisplay( )
{
    trashBagListBox.ResetContent( );  // clears the contents of the list box
    for (int i = 0; i < bag.getNumItems( ); i++)
    {
      BagItem item = bag.getItemByIndex(i);
      CString description_ = BagItem.toString( ));
      trashBagListBox.AddString( description_ );
    }
 }
...
{
   UpdateData(TRUE);
   BagItem.SetItem(weight, description);
   Bag.add(BagItem);
	updateTrashBagContentsDisplay();
   UpdateData(FALSE);
}
