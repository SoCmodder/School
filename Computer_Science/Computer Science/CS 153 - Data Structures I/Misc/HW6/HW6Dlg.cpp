...
CHW6Dlg::CHW6Dlg(CWnd* pParent /*=NULL*/)
	: CDialog(CHW6Dlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CHW6Dlg)
	m_Age = 0;
	m_GPA = 0.0;
	m_Name = _T("");
	m_messages = _T("");
...
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CHW6Dlg)
	DDX_Control(pDX, IDC_LIST_LISTBOX, m_ListBox);
	DDX_Text(pDX, IDC_EDIT_AGE, m_Age);
	DDX_Text(pDX, IDC_EDIT_GPA, m_GPA);
	DDX_Text(pDX, IDC_EDIT_NAME, m_Name);
	DDX_Text(pDX, IDC_EDIT_MESSAGEBOX, m_messages);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CHW6Dlg, CDialog)
	//{{AFX_MSG_MAP(CHW6Dlg)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	...
	ON_BN_CLICKED(IDC_BUTTON_INSERT_Student, OnBUTTONINSERTStudent)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
void CHW6Dlg::OnButtonFind() 
{
	UpdateData (TRUE);
	m_messages = StudentList.find (m_Name, m_Age, m_GPA);
	StudentList.display (m_ListBox);
	UpdateData (FALSE);
	
}

void CHW6Dlg::OnButtonFull() 
{
	UpdateData (TRUE);
...
void CHW6Dlg::OnButtonRemove() 
{
	UpdateData (TRUE);
	m_messages = StudentList.remove (m_Name, m_Age, m_GPA);
	StudentList.display (m_ListBox);
	UpdateData (FALSE);	
}

void CHW6Dlg::OnBUTTONINSERTStudent() 
{
	UpdateData (TRUE);
