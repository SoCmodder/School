...
void ParsedTokens::Execute(ProcessBag &procs)
{
   ...
   int oldstdin, oldstdout;			//The original stdin and stdout file descriptors
   ...
   if(tokens.size() > 0)
   {
      ...
      for(ii = 0; ii < tokens.size(); ++ii)
      {
         ...
         if(tokens[ii] == "|")
         {
            command[ii] = 0;							//Numchild Does not exist, yet
            pipe_array = (int ** )calloc(Numchild-1, sizeof(int *));

			for(j=0; j<Numchild-1; j++)
			{
			...
			if(command[newCommand])
            {
               ...
               procs.AddProcess(&command[newCommand]);
            }
            foundCommand = false;
         }
         else
...
bool ParsedTokens::IsPipe(char ch)
{
	return (ch=='|');
}