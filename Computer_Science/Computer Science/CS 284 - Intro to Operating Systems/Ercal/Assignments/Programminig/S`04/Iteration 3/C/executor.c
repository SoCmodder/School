...
{
    char *Command = NULL;
    char **argTable = malloc( sizeof( char * ) );
    ...
    while( (look = Parser_next(p)) != NULL )
    {
         ...
         * We start up the argument table, copy the command as the first entry
         * into the table.
         * 
         * We then continue to the next loop
         */
        if( Command == NULL )
        {
            len = strlen( look );
        ...
        /*
         * We've got a command.  Check to see if we are at a "special" token.
         * If we are then we treat them accordingly.
         */
        ...
        /*
         * We got here, so we know we have an argument.  Increase the size of
         * the argument table and copy the argument in.
         */
        numArgs++;
        len = strlen( look );
